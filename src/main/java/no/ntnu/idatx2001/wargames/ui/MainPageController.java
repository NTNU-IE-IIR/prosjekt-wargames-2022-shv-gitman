...
public class MainPageController {
    ...
    protected void startSimulation(ActionEvent actionEvent) {
        if (this.armyOne != null
                && this.armyTwo != null
                && this.armyOne.hasUnits()
                && this.armyTwo.hasUnits()
                && !(this.terrain.equals(""))) {
            Battle battle;
            ...
            if (order < 0.5) {
                battle = new Battle(this.armyOne, this.armyTwo, this.terrain);
            } else {
                battle = new Battle(this.armyTwo, this.armyOne, this.terrain);
            }

            this.placeHolderArmyOneFile = saveTempFile(this.armyOne);
            this.placeHolderArmyTwoFile = saveTempFile(this.armyTwo);

            Army winner = battle.simulate();
            updateArmyListView(armyOneListView, this.armyOne);
            updateArmyListView(armyTwoListView, this.armyTwo);
    ...
    protected void restartSimulation(ActionEvent actionEvent) {
        ...
        this.armyOne = Army.uploadArmyFromFile(this.placeHolderArmyOneFile.toString());
        this.armyTwo = Army.uploadArmyFromFile(this.placeHolderArmyTwoFile.toString());

        updateArmyListView(this.armyOneListView, this.armyOne);
        updateArmyListView(this.armyTwoListView, this.armyTwo);
    }
    ...
    public void addArmyOneUnit(ActionEvent actionEvent) {
        this.armyOne = addUnits(
                this.armyOne, armyOneListView, viewArmyOneButton,
    ...
    public void addArmyTwoUnit(ActionEvent actionEvent) {
        this.armyTwo = addUnits(
                this.armyTwo, armyTwoListView, viewArmyTwoButton,
    ...
    protected void armyOneUnitInfoButton(ActionEvent actionEvent) {
        ViewArmyDialog viewArmyDialog = new ViewArmyDialog(this.armyOne);
    ...
    protected void armyTwoUnitInfoButton(ActionEvent actionEvent) {
        ViewArmyDialog viewArmyDialog = new ViewArmyDialog(this.armyTwo);
        viewArmyDialog.showAndWait();
    }
    ...
    protected void saveArmyOneToFile(ActionEvent actionEvent) {
        if (saveConfirmationAlert(this.armyOne)) {
            if (Army.saveArmyToFile(this.armyOne.getName(), this.armyOne, "army-templates/")) {
                saveSuccessAlert(this.armyOne);
            } else {
                saveErrorAlert();
            }
        }
    ...
    protected void saveArmyTwoToFile(ActionEvent actionEvent) {
        if (saveConfirmationAlert(this.armyTwo)) {
            if (Army.saveArmyToFile(this.armyTwo.getName(), this.armyTwo, "army-templates/")) {
                saveSuccessAlert(this.armyTwo);
            } else {
                saveErrorAlert();
            }
        }
    ...
    private void loadArmyButton(String currentArmy, Button viewUnitButton, Button saveButton,
        ...
        if (armyFile != null) {
            if (currentArmy.equals("armyOne")) {
                this.armyOne = Army.uploadArmyFromFile(armyFile.toString());
                this.placeHolderArmyOneFile = armyFile;

                fileText.setText("Loaded: " + armyFile.getName());
                armyNameText.setText(this.armyOne.getName());

                updateArmyListView(armyOneListView, this.armyOne);
            } else {
                this.armyTwo = Army.uploadArmyFromFile(armyFile.toString());
                this.placeHolderArmyTwoFile = armyFile;

                fileText.setText("Loaded: " + armyFile.getName());
                armyNameText.setText(this.armyTwo.getName());
    ...
    private void saveSuccessAlert(Army army) {
        ...
        alert.setContentText(this.armyOne.getName() + " was successfully saved as \""
                + this.armyOne.getName() + ".csv\" to the army-template directory");
        alert.showAndWait();
    }
